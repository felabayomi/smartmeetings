...
function ensureScanStorageTables() {
  if (!scanStorageReady)
    scanStorageReady = pool.query(`
    ...
    CREATE INDEX IF NOT EXISTS scan_sources_owner_idx ON scan_sources(owner_id, created_at DESC);
    ALTER TABLE meetings ADD COLUMN IF NOT EXISTS scan_source_id TEXT;
    CREATE INDEX IF NOT EXISTS meetings_scan_source_idx ON meetings(scan_source_id) WHERE scan_source_id IS NOT NULL;
  `);
  return scanStorageReady;
}

async function removeExpiredUnlinkedScans(userId) {
  const expired = await pool.query(
    `DELETE FROM scan_sources source
    WHERE source.owner_id=$1 AND source.created_at < NOW() - interval '24 hours'
      AND NOT EXISTS (SELECT 1 FROM meetings WHERE scan_source_id=source.id)
    RETURNING source.blob_url`,
    [userId],
  );
  await Promise.allSettled(expired.rows.map((row) => del(row.blob_url)));
}
    ...
    for (const minutes of reminders) {
      ...
      await pool.query(
        ...
        [
          ...
          `Starts in ${label}`,
          row.id,
          `reminder:${row.id}:${minutes}:${fireAt.toISOString()}`,
        ],
      );
      const subscriptions = await pool.query(
        "SELECT * FROM push_subscriptions WHERE calendar_token = $1",
        [row.calendar_token],
      );
      const payload = JSON.stringify({
        title: `Meeting in ${label}`,
        body: row.title,
        tag: `meeting-${row.id}-${minutes}`,
        data: { url: "/app" },
      });
...
function slugify(value) {
  return (
    String(value || "meetmind-user")
      .toLowerCase()
      .trim()
      .replace(/[^a-z0-9]+/g, "-")
      .replace(/^-|-$/g, "")
      .slice(0, 40) || "meetmind-user"
  );
}
...
async function getProfileBySlug(slug) {
  ...
  const result = await pool.query(
    `SELECT * FROM scheduling_profiles WHERE slug = $1
    UNION ALL
...
function slotCandidates(profile, from = new Date(), days = 30) {
  ...
  for (let offset = 0; offset < days; offset++) {
    ...
    const end =
      endText === "24:00"
        ? fromZonedTime(
            `${addDays(date, 1).toISOString().slice(0, 10)}T00:00:00`,
            profile.timezone,
          )
        : fromZonedTime(`${dateText}T${endText}:00`, profile.timezone);
    while (addMinutes(cursor, duration) <= end) {
      if (cursor >= addHours(from, 12))
        slots.push({
          startTime: cursor.toISOString(),
          endTime: addMinutes(cursor, duration).toISOString(),
        });
      cursor = addMinutes(
        cursor,
        duration + Number(profile.buffer_minutes || 0),
      );
    }
  }
  return slots;
...
function overlapsBlackout(slot, profile) {
  ...
  return blackouts.some((blackout) => {
    ...
    const start = fromZonedTime(
      `${blackout.date}T${blackout.start}:00`,
      profile.timezone,
    );
    const end = fromZonedTime(
      `${blackout.date}T${blackout.end}:00`,
      profile.timezone,
    );
...
async function availableSlots(
  ...
  const firstOwnerDate = formatInTimeZone(
    candidates[0].startTime,
    profile.timezone,
    "yyyy-MM-dd",
  );
  const lastOwnerDate = formatInTimeZone(
    candidates[candidates.length - 1].startTime,
    profile.timezone,
    "yyyy-MM-dd",
  );
  const bookingRangeStart = fromZonedTime(
    `${firstOwnerDate}T00:00:00`,
    profile.timezone,
  ).toISOString();
  const bookingRangeEndDate = addDays(new Date(`${lastOwnerDate}T12:00:00Z`), 1)
    .toISOString()
    .slice(0, 10);
  const bookingRangeEnd = fromZonedTime(
    `${bookingRangeEndDate}T00:00:00`,
    profile.timezone,
  ).toISOString();
  const [busy, bookings] = await Promise.all([
    pool.query(
      "SELECT start_time, COALESCE(end_time, start_time + interval '30 minutes') AS end_time FROM meetings WHERE calendar_token = $1 AND ($5::int IS NULL OR id <> $5) AND start_time < ($2::timestamptz + ($4::int * interval '1 minute')) AND COALESCE(end_time, start_time + interval '30 minutes') > ($3::timestamptz - ($4::int * interval '1 minute'))",
      [
        profile.user_id,
        end,
        candidates[0].startTime,
        buffer,
        exclude.meetingId || null,
      ],
    ),
    profile.max_bookings_per_day
      ? pool.query(
          "SELECT start_time FROM bookings WHERE owner_id = $1 AND ($4::text IS NULL OR id <> $4) AND start_time >= $2 AND start_time < $3",
          [
            profile.user_id,
            bookingRangeStart,
            bookingRangeEnd,
            exclude.bookingId || null,
          ],
        )
  ...
  return candidates.filter((slot) => {
    ...
    if (
      profile.max_bookings_per_day &&
      (bookingsPerDay.get(date) || 0) >= profile.max_bookings_per_day
    )
      return false;
    return !busy.rows.some(
      (item) =>
        new Date(item.start_time) <
          addMinutes(new Date(slot.endTime), buffer) &&
        addMinutes(new Date(item.end_time), buffer) > new Date(slot.startTime),
    );
  });
}
...
function normalizeBufferMinutes(value) {
  ...
  return Number.isFinite(minutes)
    ? Math.max(0, Math.min(720, Math.round(minutes)))
    : 0;
}

function pollPayload(row, responses = []) {
  const counts = Object.fromEntries(
    (row.options || []).map((option) => [option, 0]),
  );
...
const extractedMeetingSchema = {
  ...
  properties: {
    ...
    sourceStartLocal: { type: ["string", "null"] },
    sourceEndLocal: { type: ["string", "null"] },
    sourceTimezone: { type: ["string", "null"] },
    alternateStartLocal: { type: ["string", "null"] },
    alternateTimezone: { type: ["string", "null"] },
...
function meetingValues(body, partial = false) {
  ...
  if (!partial && (typeof result.title !== "string" || !result.title.trim()))
    throw new Error("Title is required");
  if (!partial && !result.startTime) throw new Error("Start time is required");
  return result;
}

async function extractMeeting(request, response, userId) {
  ...
  if (!validImageSignature(imageBytes, mimeType)) {
    return response
      .status(400)
      .json({ error: "The file content does not match its image type" });
  }
  ...
  if (!outputText)
    return response
      .status(502)
      .json({ error: "AI returned no meeting details" });
  ...
  const localInstant = (value, timezone) => {
    if (
      !value ||
  ...
  for (const item of parsed.meetings) {
    ...
    const alternateStart = localInstant(
      item.alternateStartLocal,
      item.alternateTimezone,
    );
    if (
      sourceStart &&
      alternateStart &&
      Math.abs(sourceStart.getTime() - alternateStart.getTime()) > 5 * 60_000
    ) {
      return response.status(422).json({
        error:
          "The times and timezones shown in this image do not resolve to the same moment. Please enter this meeting manually.",
      });
    }
    const verifiedStart = sourceStart || alternateStart;
    if (verifiedStart) item.startTime = verifiedStart.toISOString();
    const sourceEnd = localInstant(item.sourceEndLocal, item.sourceTimezone);
    if (sourceEnd) item.endTime = sourceEnd.toISOString();
    if (sourceStart && validTimezone(item.sourceTimezone))
      item.timezone = item.sourceTimezone;
  }

  await ensureScanStorageTables();
  await removeExpiredUnlinkedScans(userId);
  const extension = {
    "image/jpeg": "jpg",
    "image/png": "png",
    "image/webp": "webp",
  ...
  const stored = await put(
    `scan-sources/${randomUUID()}.${extension}`,
    imageBytes,
    {
      access: "private",
  ...
  try {
    await pool.query(
      "INSERT INTO scan_sources (id,owner_id,blob_url,mime_type) VALUES ($1,$2,$3,$4)",
      [scanId, userId, stored.url, mimeType],
    );
  } catch (error) {
    await del(stored.url).catch(() => undefined);
    throw error;
  }
  parsed.meetings.forEach((item) => {
    item.sourceScanId = scanId;
    item.hasSourceImage = true;
  });
      ...
      try {
        ...
        const inserted = await client.query(
          ...
          [
            ...
            profile.timezone,
            guestName.trim(),
            `Guest: ${guestEmail}${guestTimezone ? ` • ${guestTimezone}` : ""}${notes ? `\n${notes}` : ""}`,
            "#10b981",
          ],
        );
