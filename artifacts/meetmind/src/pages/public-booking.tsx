...
export default function PublicBooking() {
  ...
  if(done&&selected&&profile)return <div className="min-h-screen bg-[#f8f9ff] grid place-items-center p-5"><div className="bg-white rounded-3xl shadow-xl border max-w-lg w-full p-8 text-center"><CheckCircle2 className="w-16 h-16 text-emerald-500 mx-auto"/><h1 className="text-3xl font-bold mt-5">You’re booked!</h1><p className="text-muted-foreground mt-2">Your meeting with {profile.displayName} is confirmed.</p><div className="rounded-2xl bg-slate-50 p-5 mt-6"><p className="font-bold">{format(selected.startTime,visitorZone)}</p><p className="text-sm text-muted-foreground mt-1">Your timezone: {visitorZone}</p><p className="text-sm text-muted-foreground">Owner timezone: {format(selected.startTime,profile.timezone)} ({profile.timezone})</p></div></div></div>;
