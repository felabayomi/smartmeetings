...
export function DayModal({ date, meetings, isOpen, onClose, onMeetingClick, onAddMeeting }: DayModalProps) {
  if (!date) return null;
      ...
      <DialogContent className="sm:max-w-md p-0 overflow-hidden bg-card border border-border shadow-2xl rounded-2xl">
        ...
        <div className="px-6 py-4 min-h-[80px] max-h-[50vh] overflow-y-auto space-y-2">
          {dayMeetings.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-8 text-center">
              ...
              <p className="text-muted-foreground font-medium text-sm">No meetings on this day</p>
              <p className="text-muted-foreground/60 text-xs mt-0.5">Tap below to schedule one</p>
            </div>
          ) : (
            dayMeetings.map((meeting) => {
              ...
              return (
                <motion.button
                  key={meeting.id}
                  whileTap={{ scale: 0.98 }}
                  onClick={() => { onClose(); onMeetingClick(meeting); }}
                  className={cn(
                    "w-full text-left flex items-stretch gap-3 p-3 rounded-xl border transition-all hover:shadow-sm group",
                    past ? "opacity-60" : "hover:border-border"
                  )}
                  style={{ borderColor: `${color}30`, backgroundColor: `${color}08` }}
                >
                  {/* Color stripe */}
                  <div className="w-1 flex-shrink-0 rounded-full self-stretch" style={{ backgroundColor: color }} />
                  {/* Content */}
                  <div className="flex-1 min-w-0">
                    <p className={cn("font-semibold text-sm leading-tight truncate", past ? "text-muted-foreground" : "text-foreground")}>
                      {meeting.title}
                    </p>
                    ...
                    {meeting.organizer && (
                      <p className="text-xs text-muted-foreground mt-0.5 truncate">{meeting.organizer}</p>
                    )}
                  </div>
                  <ChevronRight className="w-4 h-4 text-muted-foreground/40 self-center flex-shrink-0 group-hover:text-muted-foreground transition-colors" />
                </motion.button>
              );
            })
          )}
        ...
        <div className="px-6 pb-6 pt-2 border-t border-border">
          <Button
            ...
            onClick={() => { onClose(); onAddMeeting(date); }}
          >
