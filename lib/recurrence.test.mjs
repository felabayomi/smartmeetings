...
import { normalizeRecurrence, expandRecurrence } from './recurrence.cjs';
import { createSeries, excludeOccurrence, materializeSeries } from './recurrence-store.cjs';
import { createRequire } from 'node:module';

test('Vercel CommonJS loader can require both recurrence modules', () => {
  const require = createRequire(import.meta.url);
  assert.equal(typeof require('./recurrence.cjs').expandRecurrence, 'function');
  assert.equal(typeof require('./recurrence-store.cjs').materializeSeries, 'function');
  assert.equal(require('./recurrence.cjs').RecurrenceError.name, 'RecurrenceError');
});
